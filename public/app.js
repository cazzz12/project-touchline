...
import {opponentCoachView} from './opponent-coach.js';
import {liveFitness,matchInjury,matchInjuryView} from './match-injuries.js';
...
function liveModal(){
  ...
  const computerCoach=full?report.detail.opponentCoach:pending.opponentCoach,opponentName=game.clubs[home===0?away:home].name;
  const timeline=[...(computerCoach?.changes||[]).map(c=>({minute:c.minute,text:`Wissel: ${c.outName} → ${c.inName} · lichte tik`,sub:`Computercoach · ${opponentName}`,type:'coaching'})),...events.map(e=>({minute:e.minute,text:`${e.player} · ${e.type==='injury'?'LICHTE TIK':e.type==='goal'?'DOELPUNT':e.type==='yellow'?'GELE KAART':e.type==='red'?(e.secondYellow?'TWEEDE GEEL · ROOD':'RODE KAART'):e.type==='save'?(e.keeper?`REDDING ${e.keeper}`:'SCHOT GESTOPT'):'SCHOT NAAST'}`,sub:`${game.clubs[e.side===0?home:away].name}${e.xg===undefined?'':` · ${e.xg} xG`}`,type:e.type})),...coaching.map(e=>({...e,type:'coaching',sub:'Managerbeslissing'}))].sort((a,b)=>b.minute-a.minute);
...
$('#app').addEventListener('click',e=>{
  ...
  if(action==='injury-sub'&&game.pending?.paused&&game.pending.subs<3){const id=game.pending.selection.find(id=>matchInjury(game,id));if(id&&game.pending.bench.length){$('#sub-out').value=id;$('#sub-in').focus();}return;}
