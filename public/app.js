...
function transfers(){return transferSection==='scouting'?scouting():shell(clubTransfersView(game,transferFilters,bidCandidate),'Scouting & transfers','BOUW AAN JE SELECTIE');}
...
function backupAction(action){
  ...
  if(action==='confirm-import'&&backupCandidate){
    try{
      ...
      clearInterval(timer);game=restored;saleCandidate=null;fitnessProposal=null;bidCandidate=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
...
$('#app').addEventListener('click',e=>{
  ...
  if(!game)return;
  if(b.dataset.transferSection){transferSection=b.dataset.transferSection;bidCandidate=null;message='';render();return;}
  if(b.dataset.clubBid){bidCandidate={seller:Number(b.dataset.seller),id:b.dataset.clubBid};message='';render();$('#club-bid-form input[name="amount"]')?.focus();return;}
  ...
  if(b.dataset.action==='update-rosters'){
    try{
      ...
      game=updated;fitnessProposal=null;bidCandidate=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
