...
let selectedClub=0,playerSearch='',roleFilter='all',officeSection='finances',saleCandidate=null;
...
const tabs={overview:['◈','Overzicht'],squad:['♟','Selectie'],clubs:['⚑','Clubs'],prematch:['▷','Voorbeschouwing'],tactics:['◎','Tactiek'],training:['↗','Training'],transfers:['◇','Scouting & transfers'],league:['▤','Competitie'],cluboffice:['▧','Clubzaken'],career:['★','Carrière'],progress:['↓','Voortgang']};
...
function career(){return shell(careerView(game),'Jouw carrière','ELK SEIZOEN TELT');}
...
function backupAction(action){
  ...
  if(action==='confirm-import'&&backupCandidate){
    try{
      ...
      clearInterval(timer);game=restored;saleCandidate=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
...
function liveModal(){
  ...
  return `<div class="modal-backdrop"><section class="modal live-modal" role="dialog" aria-modal="true" aria-label="Wedstrijd"><div class="match-heading"><p class="kicker">${full?'FULL TIME':pending.paused?(minute===45?'RUST · JIJ BENT AAN ZET':'GEPAUZEERD · JIJ BENT AAN ZET'):'LIVE · MINI LEAGUE'}</p><span class="live-clock">${minute}'</span></div><div class="match-progress"><span style="width:${100*minute/90}%"></span></div><div class="score"><span class="score-club">${clubLogo(game.clubs[home].name)}${esc(game.clubs[home].name)}</span><strong>${stats[0].goals} : ${stats[1].goals}</strong><span class="score-club">${clubLogo(game.clubs[away].name)}${esc(game.clubs[away].name)}</span></div>
...
function render(){
  ...
  $('#app').innerHTML=`<div id="storage-status">${storageNotice()}</div>`+({overview,squad,clubs,prematch,tactics,training,transfers,league,cluboffice,career,progress})[tab]()+(!hideMatch?liveModal():'');
}
$('#app').addEventListener('submit',e=>{
  if(['manager-form','auto-form','development-form'].includes(e.target.id)){
    ...
    if(e.target.id==='development-form')message=developPlayer(game,data.get('playerId'),data.get('attribute'))?'Individuele training afgerond.':'Deze training is nu niet mogelijk.';
    save();render();return;
  }
...
$('#app').addEventListener('click',e=>{
  ...
  if(b.dataset.action==='update-rosters'){
    try{
      ...
      game=updated;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
  ...
  if(action==='play'){const match=beginMatch(game);hideMatch=false;message=match?'':'Verleng eerst de verlopen contracten van je wedstrijdselectie via Clubzaken.';save();render();}
