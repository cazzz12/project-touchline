...
function backupAction(action){
  ...
  if(action==='confirm-import'&&backupCandidate){
    try{
      ...
      clearInterval(timer);game=restored;saleCandidate=null;fitnessProposal=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
...
function liveModal(){
  ...
  ${full?fitnessReport(report.medicalReport):''}
...
$('#app').addEventListener('click',e=>{
  ...
  if(b.dataset.action==='update-rosters'){
    try{
      ...
      game=updated;fitnessProposal=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
  ...
  if(b.dataset.tab){fitnessProposal=null;tab=b.dataset.tab;message='';render();return;}
  ...
  if(action==='play'){const match=beginMatch(game);hideMatch=false;message=match?'':unavailableSelection(game).length?'Vervang eerst de geblesseerde spelers via Selectie.':'Verleng eerst de verlopen contracten van je wedstrijdselectie via Clubzaken.';save();render();}
