...
function transfers(){return transferSection==='scouting'?scouting():shell(transferSection==='incoming'?incomingTransfersView(game,incomingCandidate):['search','shortlist'].includes(transferSection)?scoutingDeskView(game,transferSection,scoutingComparison):clubTransfersView(game,transferFilters,bidCandidate,scoutingComparison),'Scouting & transfers','BOUW AAN JE SELECTIE');}
...
$('#app').addEventListener('click',e=>{
  ...
  if(b.dataset.transferSection){if(!['search','shortlist','clubs','incoming','scouting'].includes(b.dataset.transferSection))return;transferSection=b.dataset.transferSection;scoutingComparison=null;incomingCandidate=null;bidCandidate=null;message='';render();return;}
