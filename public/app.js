...
function liveModal(){
  ...
  const timeline=[...events.map(e=>({minute:e.minute,text:`${e.player} · ${e.type==='goal'?'DOELPUNT':e.type==='save'?(e.keeper?`REDDING ${e.keeper}`:'SCHOT GESTOPT'):'SCHOT NAAST'}`,sub:`${game.clubs[e.side===0?home:away].name} · ${e.xg} xG`,type:e.type})),...coaching.map(e=>({...e,type:'coaching',sub:'Managerbeslissing'}))].sort((a,b)=>b.minute-a.minute);
  ...
  ${full?keeperReportView(report.detail)+fitnessReport(report.medicalReport):''}
