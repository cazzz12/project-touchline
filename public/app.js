...
let incomingCandidate=null;
let transferSection='clubs',transferFilters={seller:1,role:'all',search:''},bidCandidate=null;
function transfers(){return transferSection==='scouting'?scouting():shell(transferSection==='incoming'?incomingTransfersView(game,incomingCandidate):clubTransfersView(game,transferFilters,bidCandidate),'Scouting & transfers','BOUW AAN JE SELECTIE');}
...
function backupAction(action){
  ...
  if(action==='confirm-import'&&backupCandidate){
    try{
      ...
      clearInterval(timer);game=restored;saleCandidate=null;fitnessProposal=null;bidCandidate=null;incomingCandidate=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
...
$('#app').addEventListener('click',e=>{
  ...
  if(b.dataset.transferSection){if(!['clubs','incoming','scouting'].includes(b.dataset.transferSection))return;transferSection=b.dataset.transferSection;incomingCandidate=null;bidCandidate=null;message='';render();return;}
  ...
  if(b.dataset.action==='update-rosters'){
    try{
      ...
      game=updated;fitnessProposal=null;bidCandidate=null;incomingCandidate=null;backupCandidate=null;importSequence++;storageError='';unreadableRaw=null;
