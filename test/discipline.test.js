...
function redGame(){const g=newGame();g.season=3;g.captainId=g.lineupIds[10];for(const c of Object.values(g.management.contracts))c.untilSeason=5;beginMatch(g);advance(g,64);return g;}
function makeLegacy(g){for(const key of ['developmentRules','disciplineRules','opponentKeeperMinutes','opponentPlayed','opponentStarted','bookings','dismissed','abandoned'])delete g.pending[key];for(const s of g.pending.stats){delete s.yellowCards;delete s.redCards;}}
