...
import {developPlayer,sellPlayer,saleOffer} from '../public/management.js';
...
test('v0.6 pending matches finish with exactly the previous results, money, events and player records',()=>{
  ...
  for(const [club,expected] of [['Ajax','ecd366ca0d14f47d439b56e618e2b5285e036aacedcff738feae185dde1acd9f'],['AZ','65b1e3eeb0d264a6070ea55245728fe11dac4f62076ce007cc9b78f6054b94c5']]){
    let g=newGame(club);beginMatch(g);delete g.pending.opponentCoach;delete g.pending.injuryRules;delete g.pending.keeperRules;delete g.pending.keeperMinutes;
    for(const key of ['stadiumRules','stadiumGate','leagueMarketRules','reputationRules','developmentRules','disciplineRules','opponentKeeperMinutes','opponentPlayed','opponentStarted','bookings','dismissed','abandoned'])delete g.pending[key];
...
test('keeper skills and career records follow a transfer without a new identity',()=>{
  ...
  skills(p,91);const buyer=[1,2,3,4,5].find(i=>saleOffer(g,id,i));assert.ok(buyer);assert.equal(sellPlayer(g,id,buyer),true);
  assert.equal(g.clubs[buyer].players.find(p=>p.id===id).reflexes,91);assert.deepEqual(g.management.playerStats[id],stats);
});
