...
test('v0.13 live matches retain exact club rosters, finances and match history after reload',()=>{
  ...
  for(const [club,expected] of [['Ajax','83c7350b29e314f0bc356d2cf240a7a0bfa9ac145d15ddbd0d5edfea279ea75b'],['PSV','a8ebd6b8d15f9f3ca3f643f3a8b438e3f3292a6f4d7e4412dc9b83b7b2fda2b8']]){
    let g=newGame(club);beginMatch(g);delete g.pending.opponentCoach;delete g.pending.injuryRules;delete g.pending.stadiumRules;delete g.pending.stadiumGate;delete g.pending.leagueMarketRules;delete g.leagueMarket;advance(g,32);g=parseBackup(exportBackup(g));advance(g);
